...
public class MainActivity extends Activity {
    ...
    String RefreshStatementFunc = " window.refresh_statement = function() { $('select#P6_DATE').empty(); $('div#attendance th').slice(1).each(function(index) { $('select#P6_DATE').append(\"<option value='\" + index + \"'>\" + $(this).text() + \"</option>\"); " + HideWarnings + HideColumns + " }); }; ";
    ...
    String OnLoaded = AppendSelectNode + HideWarnings + RefreshStatementFunc + " $('select#P6_DATE').change(function() { " + OnDateChanged + " }); ";
